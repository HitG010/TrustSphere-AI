...
export default function LogoUpload() {
    ...
    const handleGradCamPredict = async () => {
        if (!file) {
            setError('Please select an image file');
            return;
        }
        ...
        try {
            const response = await axios.post('http://localhost:5000/api/predict-authenticity', formData, {
                headers: { 'Content-Type': 'multipart/form-data' },
            });
            setGradCamResult(response.data);
        } catch {
            setError('Failed to predict authenticity');
        } finally {
    ...
    return (
        <div className="bg-gray-950 min-h-screen px-6 py-10 text-white">
            ...
            <div className={`flex ${logoResult || ocrResult || gradCamResult ? 'flex-col md:flex-row' : 'items-center justify-center'} gap-8 transition-all duration-500`}>
                <div className={`${logoResult || ocrResult || gradCamResult ? 'w-full md:w-1/2' : 'w-full max-w-md'} space-y-6 transition-all duration-500`}>
                    <div
                    ...
                    <button
                        ...
                        className={`py-3 rounded-full w-full text-lg font-semibold transition-colors ${
                            loadingLogo ? 'bg-cyan-800 cursor-not-allowed text-gray-300' : 'bg-cyan-500 hover:bg-cyan-400 text-black'
                        }`}
                    >
                    ...
                    <button
                        onClick={handleGradCamPredict}
                        disabled={loadingGradCam}
                        className="w-full py-3 mt-4 bg-purple-600 hover:bg-purple-500 rounded-xl font-semibold transition-all"
                    >
                        {loadingGradCam ? 'Running Grad-CAM...' : 'Run Grad-CAM Authenticity Check'}
                    </button>

                    {error && <p className="text-red-500 font-medium">{error}</p>}
                </div>

                {(logoResult || ocrResult || gradCamResult) && (
                    <div className="w-full md:w-1/2 space-y-6">
                        ...
                        {gradCamResult?.status === 'success' && (
                            <div className="bg-gray-900 border border-purple-700 rounded-xl p-6 shadow-inner">
                                <h3 className="text-xl font-bold mb-2 text-purple-300">Grad-CAM Authenticity Result</h3>
                                {/* <div><strong>Predicted Class:</strong> {gradCamResult.predicted_class}</div>
                                <div><strong>Similarity Score:</strong> {gradCamResult.similarity_score}</div> */}
                                <div className="mt-3">
                                    <img
                                        src={`http://localhost:5000/gradcam/${gradCamResult.gradcam_image_path.split('/').pop()}`}
                                        alt="Grad-CAM"
                                        className="max-w-[600px] w-full rounded-xl border border-purple-600 mt-2 object-contain"
                                    />
